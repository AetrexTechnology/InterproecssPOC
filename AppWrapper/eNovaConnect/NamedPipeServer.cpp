...
int NamedPipeServer::Listen() {
    ...
    while (mIsRun) {
        ...
        if (mHPipe == INVALID_HANDLE_VALUE) {
            DBOUT("NamedPipeServer::Read, CreateNamedPipe failed, GLE=" << GetLastError() << std::endl);
            return -1;
        }
    ...
    while (mIsRun) {
        ...
        if (!fSuccess || cbBytesRead == 0) {
            if (GetLastError() == ERROR_BROKEN_PIPE) {
                DBOUT("NamedPipeServer::Read, client disconnected." << std::endl);
            } else {
                DBOUT("NamedPipeServer::Read, ReadFile failed, GLE=" << GetLastError() << std::endl);
            }
            break;
        }
        ...
        if (mRequestHandler) {
            ...
            if (!response.empty()) {
                try {
                    ...
                    DBOUT("ERROR DURING SENDING RESPONSE FROM NamedPipeServer: " << e.what() << "\n");
                    return -1;
                }
            }
        }
...
void NamedPipeServer::sendResponse(const char *responseMessage) {
    ...
    if (!fSuccess || cbReplyBytes != cbWritten) {
        DBOUT("NamedPipeServer::Read, WriteFile failed, GLE=" << GetLastError() << std::endl);
