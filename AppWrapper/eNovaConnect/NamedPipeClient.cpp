...
std::string NamedPipeClient::sendRequest(const char *requestMessage, bool waitForResponse) {
    ...
    while (true) {
        ...
        if (GetLastError() != ERROR_PIPE_BUSY) {
            DBOUT("NamedPipeClient::sendRequest, Could not open pipe, GLE=" << GetLastError() << std::endl);
        ...
        if (!WaitNamedPipe(lpszPipename, 2000)) {
            DBOUT("Could not open pipe: 20 second wait timed out."  << std::endl);
    ...
    if (!fSuccess) {
        DBOUT("NamedPipeClient::sendRequest, SetNamedPipeHandleState failed, GLE=" << GetLastError() << std::endl);
    ...
    if (!fSuccess) {
        DBOUT("NamedPipeClient::sendRequest, WriteFile to pipe failed, GLE=" << GetLastError() << std::endl);
    ...
    if (waitForResponse) {
        ...
        if (!fSuccess) {
            DBOUT("NamedPipeClient::sendRequest, ReadFile from pipe failed, GLE=" << GetLastError() << std::endl);
